...
<html>
<head>
<title><?php echo $config['site_title'].' '.$lang['ADMIN'].' - '.$lang['EDIT'].' '.$lang['POST']; ?></title>
...
<table width="850" cellpadding="0" cellspacing="0" border="0" align="center">
  <tr>
    <td valign="middle" class="heading"><?php echo $lang['EDIT'].' '.$lang['POST']; ?></td>
  </tr>
  <tr>
    <td bgcolor="#333333"><img src="images/dot.gif" width="1" height="1" alt=""></td>
  </tr>
...
<table width="850" cellpadding="0" cellspacing="0" border="0" style="border:1px solid #CCCCCC;" align="center">
  <tr>
    <td align="center" bgcolor="#F6F6F6" style="padding:15px;"><div align="right">
        <form action="post_delete.php" method="post" name="f1" id="f1">
          <div align="right"> <br>
          </div>
          <table width="99%" border="0" align="center" cellpadding="2" cellspacing="0">
            <tr bgcolor="#344167">
              ...
              <td height="30"><span class="rowheader"><?php echo $lang['POST'].' '.$lang['TITLE']; ?></span></td>
              ...
              <td width="140" height="30"><span class="rowheader"><?php echo $lang['CATEGORY']; ?></span></td>
...
$query = "SELECT 1 FROM ".$config['db']['pre']."posts";
...
$query = "SELECT post_id,post_title,user_id,post_date,cat_id FROM ".$config['db']['pre']."posts ORDER BY post_id DESC ".$limit;
$query_result = mysql_query($query);
while ($info = @mysql_fetch_array($query_result))
{
	$posts[$info['post_id']] = $info;
	
	if($author_where == '')
	{
		$author_where = "user_id='".$info['user_id']."'";
	}
	else
	{
		$author_where.= " OR user_id='".$info['user_id']."'";
	}
}

if($author_where != '')
{
	$query = "SELECT user_id,username FROM ".$config['db']['pre']."users WHERE ".$author_where;
...
?>
            <tr bgcolor="<?php echo $colour; ?>">
              <td width="50" height="25" align="center"><input type="checkbox" name="list[]" id="list[]" value="<?php echo $info['post_id']; ?>"></td>
              <td height="25"><span class="style5"><?php echo stripslashes($info['post_title']); ?></span></td>
              <td height="25"><span class="style5"><?php echo stripslashes($users[$info['user_id']]); ?></span></td>
              <td height="25"><span class="style5"><?php echo stripslashes(date('d/m/Y',$info['post_date'])); ?></span></td>
              <td height="25"><span class="style5"><?php echo stripslashes($cats[$info['cat_id']]['cat_title']); ?></span></td>
            <td height="25"><table border="0" cellpadding="0" cellspacing="0">
                  <tr>
                    <td><select name="amenu<?php echo $info['post_id']; ?>" id="amenu<?php echo $info['post_id']; ?>" onChange="MM_jumpMenu('parent',this,0)" style="width:145px">
                        <option value=""><?php echo $lang['OPTIONS']; ?></option>
                        <option value="post_edit.php?id=<?php echo $info['post_id']; ?>"><?php echo $lang['VIEW']."/".$lang['EDIT'].' '.$lang['POST']; ?></option>
                        <option value="post_delete.php?id=<?php echo $info['post_id']; ?>"><?php echo $lang['DELETE'].' '.$lang['POST']; ?></option>
                      </select>
                    </td>
                  </tr>
              </table></td>
...
<?php } ?>
          ...
          <div align="left"> <br>
              <table width="99%"  border="0" align="center" cellpadding="2" cellspacing="0">
                <tr>
                  <td width="200" valign="middle"><?php echo $lang['WITHSELECTED']; ?>:&nbsp;<a href="#" onclick="document.f1.action='post_edit.php'; checkBoxes();"><img src="images/button_edit.gif" width="12" height="13" border="0"></a>
